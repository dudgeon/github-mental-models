---
title: "Git Literacy for Product Managers Using Claude Code"
version: "1.0"
...
After completing this training (~20 minutes, self-paced), the learner can:
 ...
 2. **Articulate the tradeoffs** — what you give up and what you gain moving to a Git-based workflow, especially in the context of AI-assisted development

 3. **Explain** what a repository is and find their team's repos on GitHub.com

 4. **Describe** what a commit represents and why commit messages matter

 5. **Explain** why work happens on branches instead of directly on `main`

 6. **Describe** the purpose of a pull request and who is involved

 7. **Prompt** Claude Code to perform common Git operations and understand what it's doing

 8. **Navigate a multi-repo workspace** in VS Code and understand that Claude Code can traverse across repos when given permission

 9. **Recognize** when Git is in an error state (merge conflict, detached HEAD, uncommitted changes blocking an operation) and describe the situation clearly enough to get help

10. **Know** what types of files belong in GitHub and which don't
...
**Q:** You make changes to code on your laptop using Claude Code and then close your laptop for the day. Can your teammate see those changes on GitHub.com?
**A:** Not unless you pushed them. Your changes are local until you explicitly share them.

**Q:** Why would anyone choose this model over Google Docs?
**A:** Because it gives you safe parallel work, precise version history, structured review — and because AI coding agents are built around Git, so it's the only way to fully collaborate with them.
...
2. Your team's repos live under your organization – click the org name in the left sidebar

3. Search by project name. Repos are named in lowercase with hyphens: `card-servicing-api`, `chat-experience-frontend`
...
**Q:** Someone says "check the repo." Where do you go?
**A:** GitHub.com, to look at the remote copy.

**Q:** Someone says "I cloned the repo." What did they do?
**A:** Downloaded a copy of the project from GitHub.com to their laptop so they can work on it locally.
...
2. **Creates the commit** (`git commit -m "..."`) — records the save point with your message
...
**Q:** You ask Claude Code to make a change and it does. Is that change saved to Git's history?
**A:** No. The files on your laptop are modified, but it's not a commit until you explicitly tell Claude Code to commit.

**Q:** After you commit, can your teammate see the change on GitHub.com?
**A:** Not yet — you also need to push.
...
3. When ready, propose merging your branch back into `main` via a pull request (next module)

4. After review and approval, your branch merges in
...
**Q:** Your teammate says "I pushed to my branch." Does that mean their changes are on `main`?
**A:** No. They're on their branch on GitHub.com. They need a pull request to get those changes into `main`.

**Q:** You're on branch `fix-login-bug` and a file looks different than expected. Is something wrong?
**A:** Probably not — files reflect whichever branch you're on. Check if you need to switch branches.
...
2. **Reviewers examine it.** They see every line added (green) and removed (red), comment on specific lines, and give a decision: Approve or Request Changes.

3. **You revise if needed.** Make more changes in VS Code, commit, push. The PR updates automatically.

4. **Someone merges it.** After approval, one click brings your changes into `main`.

5. **The branch is deleted.** Its work is done. The commits live on in `main`'s history.
...
2. Creates the PR via GitHub CLI, targeting `main`

3. Returns the PR URL – click through to add reviewers and monitor feedback
...
**Q:** A reviewer comments asking you to change some wording. What do you do?
**A:** Go to VS Code, ask Claude Code to make the change, commit, push. The PR updates automatically.

**Q:** After merge, where do your changes live?
**A:** On `main` — the official version.
...
**Q:** You have a PRD in Google Docs that you want Claude Code to reference while it builds a feature. What do you do?
**A:** Create a markdown version and put it in the repo. Claude Code can then read it as context.

**Q:** Your teammate asks you to commit an Excel spreadsheet to the repo. Is that a good idea?
**A:** Generally no — GitHub can't show meaningful diffs for Excel files. Keep it in Google Sheets and, if the data is needed in the repo, export it as CSV.
...
2. Repeat for additional repos

3. **File → Save Workspace As…** — saves the arrangement so you can reopen it later
...
**Q:** You have the API repo and the frontend repo open in the same VS Code workspace. You ask Claude Code to make a change that involves both. How many commits will that produce?
**A:** At least two — one in each repo. Git operations are per-repo.

**Q:** Can Claude Code read a file in a repo you haven't opened in your workspace?
**A:** No — it needs the repo to be cloned to your laptop and open in VS Code.

### Module 9: Your Daily Workflow and Troubleshooting (3 minutes)
...
**Q:** You've been working for an hour. Claude Code has made changes to files. You haven't committed. Your laptop crashes. What happened to your work?
**A:** Gone. Uncommitted changes are just modified files on your laptop — they're not in Git's history. Commit regularly.

**Q:** You open VS Code and the bottom-left says `main`. Should you start making changes?
**A:** No — create a feature branch first, so your work doesn't go directly on the official version.
